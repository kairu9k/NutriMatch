...
- [ ] `find-rnd.vue` — client-side RND search/browse (pairs with Phase 3 matching endpoint)
- [ ] `book-appointment.vue` — booking flow from a selected RND
- [ ] `client-dashboard.vue` — client's own home dashboard (distinct from rnd-dashboard/admin-dashboard, don't confuse with `client-management.vue` which is admin-facing)
