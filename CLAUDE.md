...
- `backend/` — **Django project** (`nutrimatch_api/` settings module + 7 domain apps, all 21 tables from `vault/database.txt` modeled and migrated as of 2026-09-04). Has its own `.venv/` (Python 3.13, activate via `./.venv/Scripts/python.exe` on Windows — the `python`/`python3`/`py` aliases on this machine are inconsistent, always invoke the venv's own executable directly rather than relying on `activate`). `.env` holds local secrets (gitignored), `.env.example` is the template. App layout mirrors the schema's own 6 functional groups:
  ...
  - `accounts`: auth (`/api/auth/*` — login, refresh, me, register/client, register/rnd) + admin user management (`/api/admin/users/*`). Registering sends a 6-digit `EmailVerificationCode` via Gmail SMTP (`/api/auth/verify-email/`, `/api/auth/verify-email/resend/`) and login is refused (`AuthenticationFailed`) until `User.email_verified_at` is set — same OTP pattern as the password-reset flow. Accounts that existed before this feature (2026-09-10) were grandfathered in via a data migration (`accounts/migrations/0004_backfill_email_verified_at.py`), so only new registrations go through it.
  ...
  - `scheduling` also gained `services.py` in Phase 4 — wired into `RndAppointmentConfirmView`: confirming a video appointment auto-creates a `ConsultationSession` + video room. Originally used `DailyCoVideoService` (Daily.co); switched to `JitsiVideoService` (2026-09-10) because Daily.co's free tier gates its call UI behind having a payment method on file, and the user has neither a card nor budget. Jitsi's public `meet.jit.si` server needs no API key/account — room creation is just generating an unguessable room name (`nm-appt-{id}-{random}`), no HTTP call involved. `ConsultationSession.VideoProvider` still lists `daily_co` as a choice (kept for the enum/schema, not actively used). **Security invariant, test-verified**: `host_url` (grants RND/owner controls) must never appear in any client-facing serialized response — only `participant_url` (via `Appointment.video_session_url`) is client-visible. On Jitsi's public server `host_url` and `participant_url` are actually the same URL (no host/participant distinction), which removes the leak risk entirely rather than requiring care to avoid it. If you touch `ConsultationSessionSerializer` or add new appointment/session serializers, re-verify this doesn't regress. Jitsi's tradeoff: no auth on the public server, so access control is URL secrecy — acceptable for a capstone demo per the user's explicit call, and `ConsultationRoom.vue` already gates the URL behind an authenticated appointment lookup before revealing it.
